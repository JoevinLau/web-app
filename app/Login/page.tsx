"use client";
import { useState } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { ArrowLeft } from "lucide-react"; 

export default function LoginPage() {
    const router = useRouter();
    const [error, setError] = useState("");

    async function loginhandling(formData: FormData) {
        setError("");
        const email = formData.get("email");
        const password = formData.get("password");

        // Hardcoded Credentials
        const dbemail = "edric@gmail.com";
        const dbpassword = "1234567890";

        if (email === dbemail && password === dbpassword) {
            alert("Welcome to LuckyBet!");
            router.push("/Games"); // Redirects to Game Lobby
        } else {
            setError("Invalid email or password. Please try again.");
        }
    }

    return (
        <div className="min-h-screen bg-[#0b1120] flex flex-col items-center justify-center p-4 font-sans">
            
            <Link href="/" className="absolute top-6 left-6 text-stone-400 hover:text-white flex items-center gap-2 transition-colors">
                 <ArrowLeft className="w-4 h-4"/> Back to Home
            </Link>

            <div className="w-full max-w-md bg-[#1a2333] border border-stone-800 rounded-2xl p-8 shadow-2xl">
                <h2 className="text-3xl text-center text-white font-black uppercase italic mb-2">Welcome Back</h2>
                <p className="text-center text-stone-400 mb-6 text-sm">Login to continue playing</p>

                {error && (
                    <div className="bg-red-500/10 border border-red-500/50 text-red-400 text-center text-sm py-2 rounded mb-4">
                        {error}
                    </div>
                )}

                <form action={loginhandling} className="space-y-5">
                    <div>
                        <label className="block text-xs font-bold text-stone-500 uppercase mb-1">Email</label>
                        <input
                            type="email"
                            name="email"
                            required
                            className="w-full px-4 py-3 bg-[#0f1728] border border-stone-700 rounded-lg text-white outline-none focus:border-[#eab308] transition-colors"
                            placeholder="example@gmail.com"
                        />
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-stone-500 uppercase mb-1">Password</label>
                        <input
                            type="password"
                            name="password"
                            required
                            className="w-full px-4 py-3 bg-[#0f1728] border border-stone-700 rounded-lg text-white outline-none focus:border-[#eab308] transition-colors"
                            placeholder="••••••••"
                        />
                    </div>

                    <button 
                        type="submit" 
                        className="w-full py-3 bg-[#eab308] hover:bg-[#ca8a04] text-black font-bold rounded-lg shadow-lg shadow-yellow-900/20 transition-all active:scale-95"
                    >
                        LOGIN
                    </button>
                </form>

                <div className="mt-6 text-center text-sm text-stone-500">
                    Don't have an account?{" "}
                    <Link href="/signup" className="text-[#eab308] hover:text-[#ca8a04] font-bold hover:underline">
                        Sign Up
                    </Link>
                </div>
            </div>
        </div>
    );
}